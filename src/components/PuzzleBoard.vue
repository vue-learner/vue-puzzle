...
export default {
  ...
  setup() {
    ...
    // api
    onMounted(() => {
      console.log(sourceImg, puzzleCanvas);
      console.log(puzzleCanvas.value)
    })
    return {
      sourceImg,
      puzzleCanvas
  ...
  mounted() {
    ...
    if (this.isImage) {
      ...
      this.sourceImg.addEventListener('play', () => {
        this.isTouchNeeded = false
      })
    }
    const loop = () => {
      ...
      if (this.sourceImg == null || this.sourceImg.readyState < 3) {
        requestAnimationFrame(loop)
        return
      }
      const sourceImg = this.sourceImg
      const canvas = this.puzzleCanvas
    ...
    src() {
      if (this.isImage) {
        ...
        this.$nextTick(() => {
          ...
          this.sourceImg.addEventListener('load', () => {
            this._loadImageToCanvas()
          })
        })
      }
  ...
  methods: {
    ...
    slide(idx) {
      ...
      // this.$set(this, 'blocks', this.board.blocks.concat())
